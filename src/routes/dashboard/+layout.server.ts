import { redirect } from '@sveltejs/kit';
import type { LayoutServerLoad } from './$types';
import { requireSession } from '$lib/server/authz';
import {
	countUnreadNotifications,
	listUserNotifications,
	processTherapySessionReminders
} from '$lib/server/notifications';
import { isAppRole, type AppRole } from '$lib/roles';
import { userMustChangePassword } from '$lib/server/user-security';

export const load: LayoutServerLoad = async (event) => {
	const user = requireSession(event);
	const role = (isAppRole(user.role) ? user.role : 'patient') as AppRole;

	if (await userMustChangePassword(user.id)) {
		throw redirect(302, '/auth/change-password');
	}

	if (role === 'patient' || role === 'therapist') {
		await processTherapySessionReminders();
	}

	return {
		user: {
			id: user.id,
			name: user.name,
			email: user.email,
			role
		},
		notifications: await listUserNotifications(user.id),
		unreadNotificationCount: await countUnreadNotifications(user.id)
	};
};
